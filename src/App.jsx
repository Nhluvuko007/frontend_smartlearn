...
function App() {
  ...
  // GATEWAY VIEW: Render Login / Registration panel if session doesn't exist
  if (!user) {
    return (
  ...
  // PLATFORM VIEW 1: Active Study Session UI
  if (currentDeck) {
    ...
    return (
      <div className="container">
        <button className="btn" style={{ marginBottom: '1rem', background: '#64748b' }} onClick={() => { setCurrentDeck(null); loadDecks(); }}>
          ← Back to Dashboard
        </button>
        ...
        {isSessionFinished ? (
          <div style={{ textAlign: 'center', padding: '3rem', background: 'white', borderRadius: '12px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            ...
            <button className="btn" onClick={() => { setCurrentDeck(null); loadDecks(); }}>Return Home</button>
          </div>
        ) : (
          <div>
  ...
  // PLATFORM VIEW 2: Dashboard Homepage UI
  return (
    <div className="container">
      ...
      {/* Modal B: Add Flashcard Manually & AI Upload Portal */}
      {showCardModal && (
        <div style={{ background: 'rgba(0,0,0,0.5)', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
          <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ background: '#eff6ff', padding: '1rem', borderRadius: '8px', border: '1px dashed #bfdbfe', marginBottom: '1.5rem' }}>
              ...
              <p style={{ margin: '0 0 1rem 0', fontSize: '0.85rem', color: '#1e3a8a' }}>Upload a lecture PDF chapter. Our NLP framework will parse text definitions into core review cards.</p>
